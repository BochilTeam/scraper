...
export default function youtubeSearch(query: string): Promise<YoutubeSearch> {
	return new Promise<YoutubeSearch>((resolve, reject) => {
		request(
			...
			(error, response, body) => {
				...
				(
					...
					if (
						["horizontalCardListRenderer", "shelfRenderer"].includes(typeName)
					)
					...
					if (isVideo) {
						...
						results.video.push({
							...
							description:
								result.detailedMetadataSnippets?.pop().snippetText.runs.pop()
									.text || "",
					...
					if (isChannel) {
						...
						results.channel.push({
							...
							subscriberH:
								result.subscriberCountText.accessibility.accessibilityData
									.label,
