...
interface Iresyoutubedl {
    id: string
    v_id?: string
    thumbnail: string
    title: string
    video: IresultsVideoOrAudio,
    audio: IresultsVideoOrAudio
}

interface IresFetch { status: string, result: string }

// https://github.com/BochilGaming/games-wabot/blob/main/lib/y2mate.js
const servers = ['en163', 'id90']
export async function youtubedl(url: string, server: string = 'en163'): Promise<Iresyoutubedl> {
    ...
    const json: IresFetch = await (await fetch(`https://www.y2mate.com/mates/${server}/analyze/ajax`, {
        method: 'POST',
        headers: {
            'content-type': 'application/x-www-form-urlencoded; charset=UTF-8',
            cookie: '_ga=GA1.2.1405332118.1641699259; _gid=GA1.2.1117783105.1641699259',
    ...
    let video: Iresyoutubedl['video'] = {}
    let audio: Iresyoutubedl['audio'] = {}
    $('#mp4 > table > tbody > tr').each(function () {
        ...
        if (!/\.3gp/i.test(_quality)) video[quality] = {
            quality,
            fileSizeH,
    ...
    $('#mp3 > table > tbody > tr').each(function () {
        const el = $(this).find('td')
        const _quality = el.eq(0).find('a').text()
        const quality = _quality.split('(')?.[1]?.replace(')', '')?.trim()?.toLowerCase()
        const fileSizeH = el.eq(1).text()
        audio[quality] = {
            quality,
            fileSizeH,
...
export async function youtubedlv2(url: string): Promise<Iresyoutubedl> {
    ...
    const json: {
        vid: string, title: string, a: string, token: string, timeExpires: string, fn: string, links: {
            [Key: string]: {
                [Key: string]: IresLinks
            }
        }
    } = await (await fetch(urlAjax, {
        method: 'POST',
        headers: {
            'content-type': 'application/x-www-form-urlencoded; charset=UTF-8',
...
async function convert(_id: string, v_id: string, ftype: string, fquality: number): Promise<string> {
    ...
    const json: IresFetch = await (await fetch('https://www.y2mate.com/mates/convert', {
        method: 'POST',
        headers: {
            'content-type': 'application/x-www-form-urlencoded; charset=UTF-8',
...
function convertv2(url: string, v_id: string, ftype: string, fquality: string, token: string, timeExpire: number, fname: string): Promise<string> {
    return new Promise<string>(async (resolve, reject) => {
        ...
        const resServer: { c_server?: string, d_url?: string, c_status: string } = await (await fetch(url, {
            method: 'POST',
            headers: {
                'content-type': 'application/x-www-form-urlencoded; charset=UTF-8',
                origin: 'https://yt5s.com',
        ...
        const results: {
            ...
            statusCode: number,
            result: string
        } = await (await fetch(`${server}/api/json/convert`, {
            method: 'POST',
        ...
        if (results.statusCode === 200) return resolve(results.result)
            ...
            try {
                ...
                ws.on('message', function incoming(message: Buffer) {
                    const msg: { action: string, url: string } = JSON.parse(message.toString())
