...
export async function getRenderedSnapSaveUrl (url: string): Promise<RenderedSnapSave> {
  while (true) {
    ...
    if (json.status !== 1) throw new Error(json.data.identifier)
...
export function generateTokenYoutube4kdownloader (url: string) {
  function decode_max (url: string) {
    let link = url
    let _url = url
